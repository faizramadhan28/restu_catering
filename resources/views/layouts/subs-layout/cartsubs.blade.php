<div :class="cartOpen ? 'translate-x-0 ease-out' : 'translate-x-full ease-in'" class="z-50 fixed right-0 top-0 max-w-sm w-full h-full px-6 py-4 transition duration-300 transform overflow-y-auto bg-white dark:bg-gray-800 border-l-2 border-gray-300">
  ...
  <div id="cart-langganan">
    @foreach (App\Models\CartLangganan::where('user_id', Auth::id())->get() as $cart)
    @php
  ...
  <a href="{{route('subs.checkout')}}" class="flex items-center justify-center mt-4 px-3 py-2 bg-emerald-600 text-white text-sm uppercase font-medium rounded hover:bg-white focus:outline-none focus:bg-white cursor-pointer">
      <span>Checkout</span>
