                ...
                <table class="shadow-xl" id="datatable">
                    ...
                    <tbody class="bg-gray-100 dark:bg-gray-800">
                        ...
                        <td class="px-6 py-4 text-wrap">
                            ...
                            <td class="px-6 py-4 text-wrap">
                                        <div class="text-sm text-gray-900 dark:text-gray-900">{{ $subs->user_id }}</div>
                                    </td>
                            <td class="px-6 py-4 text-wrap">
                                        <div class="text-sm text-gray-900 dark:text-gray-900">{{ $subs->nama_penerima }}</div>
                                    </td>
                            <td class="px-6 py-4 text-wrap">
                                        <div class="text-sm text-gray-900 dark:text-gray-900">{{ $subs->telp_penerima }}</div>
                                    </td>
                            <td class="px-6 py-4 text-wrap">
                                    <div class="text-sm text-gray-900 dark:text-gray-900">{{ $subs->waktu_kirim }}</div>
                                </td>
                            <td class="px-6 py-4 text-wrap">
                                    <div class="text-sm text-gray-900 dark:text-gray-900">{{ $subs->waktu_berakhir }}</div>
                                </td>
                                <td class="px-6 py-4 text-wrap">
                                    <div class="text-sm text-gray-900 dark:text-gray-900">{{ $subs->bukti_pembayaran ? 'Menunggu Konfirmasi' : 'Menunggu Pembayaran' }}</div>
                                </td>
                                <td class="px-6 py-4 text-wrap">
                                    <div class="text-sm text-gray-900 dark:text-gray-900">
                                        @foreach ($statuses as $stat)
                                        @if ($stat->inisial == $subs->status)
                                        {{ $stat->nama_status }}
                                        @endif
                                        @endforeach
                                    </div>
                                    ...
                                    <td class="px-6 py-4 text-wrap">
                                        <x-dropdown align="right" width="48">
                                            ...
                                            <x-slot name="content">
                                                <x-dropdown-link :href="route('ordersubs.detail', $subs->id)">
                                                    {{ __('Details') }}
                                                </x-dropdown-link>
                                            </x-slot>
                                        </x-dropdown>
