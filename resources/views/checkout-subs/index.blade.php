...
<div class="container mx-auto p-8 bg-white dark:bg-gray-800 shadow-lg">
    ...
    <div class="flex flex-col lg:flex-row mt-8">
        <div class="w-full lg:w-1/2 order-2">
            ...
            <form class="mt-8 lg:w-3/4" method="POST" action="{{route('subs.checkout.post')}}">
                @csrf
                ...
                <div class="mt-8">
                    ...
                    <div class="flex justify-between">
                        <div class="w-3/12">
                            <div class="mt-6 flex">
                                <label class="block flex-1">
                                    <input type="time" name="time_awal" id="time_awal" class="form-input mt-1 block w-full text-gray-700" placeholder="time">
                                </label>
                            </div>
                        </div>
                        <div class="w-9/12">
                            <div class="mt-6 flex">
                                <label class="block flex-1">
                                    <input type="date" name="date_awal" id="date_awal" class="form-input mt-1 block w-full text-gray-700" placeholder="Date">
                                </label>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-8">
                    <h4 class="text-sm text-gray-500 font-medium">Waktu Pesanan Berakhir</h4>
                    <div class="flex justify-between">
                        <div class="w-3/12">
                            <div class="mt-6 flex">
                                <label class="block flex-1">
                                    <input type="time" name="time_akhir" id="time_akhir" class="form-input mt-1 block w-full text-gray-700" placeholder="time">
                                </label>
                            </div>
                        </div>
                        <div class="w-9/12">
                            <div class="mt-6 flex">
                                <label class="block flex-1">
                                    <input type="date" name="date_akhir" id="date_akhir" class="form-input mt-1 block w-full text-gray-700" placeholder="Date">
                                </label>
                            </div>
                        </div>
                    </div>
        ...
        <div class="w-full mb-8 flex-shrink-0 order-1 lg:w-1/2 lg:mb-0 lg:order-2 flex justify-end">
            <div x-data="{
                ...
                <div class="border border-gray-600 rounded-md max-w-md w-full px-4 py-3 mb-5">
                    ...
                    @php
                        $langganan = App\Models\Langganan::find($cart->langganan_id);
                    @endphp
                        <div x-data="{
                            qty:{{$cart->qty}},
                            harga:{{$langganan->harga}},
                            subsApi(method,url,callback) {
                            ...
                            <div class="flex w-full">
                                ...
                                <div class="mx-3 flex flex-col w-full">
                                    <h3 class="text-sm text-gray-600 dark:text-gray-200 mb-auto">{{$langganan->menu}}</h3>
                                    <div class="flex items-center justify-end">
                                        <div class="text-gray-600 whitespace-nowrap" x-text="'Rp. ' + harga"></div>

                                        <div class="flex items-center mx-3">
                                            <button class="text-gray-500 dark:text-gray-300 focus:outline-none focus:text-gray-600" @click="subsApi('PUT','{{route('cartsubs.more',$langganan->id).'?api_token='.Auth::user()->api_token}}',() => $parent.totalHarga = $parent.totalHarga + harga);">
                                                <svg class="h-5 w-5" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor"><path d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                            </button>
                                            <span class="text-gray-700 dark:text-gray-100 mx-2" x-text="qty"></span>
                                            <button class="text-gray-500 dark:text-gray-300 focus:outline-none focus:text-gray-600" @click="subsApi('DELETE','{{route('cartsubs.less',$langganan->id).'?api_token='.Auth::user()->api_token}}',() => $parent.totalHarga = $parent.totalHarga - harga);" >
